...
import com.hm.viewdemo.databinding.ActivityShadowLayoutBinding
...
/**
 * Created by p_dmweidu on 2025/6/21
 * Desc: 测试ShadowLayout
 */
class ShadowLayoutActivity : BaseActivity<ActivityShadowLayoutBinding>() {


    companion object {

        fun launch(context: Context) {
            val starter = Intent(context, ShadowLayoutActivity::class.java)
            context.startActivity(starter)
        }
    }

    override fun createViewBinding(): ActivityShadowLayoutBinding {
        return ActivityShadowLayoutBinding.inflate(layoutInflater)
    }

    override fun initData() {
        binding.flContainer.setOnClickListener {
            Toast.makeText(
                this@ShadowLayoutActivity,
                "点击了fl_container",
                Toast.LENGTH_SHORT
            ).show()
        }
