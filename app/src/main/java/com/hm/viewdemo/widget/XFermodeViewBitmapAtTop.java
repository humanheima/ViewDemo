 ...
 * Desc:测试Bitmap渐隐透下去的效果，Bitmap在上面src,渐隐图层在下面dst
 * 使用 src_atop 模式：
 * 丢弃源图像像素没有覆盖目标图像像素区域的源图像像素。在目标图像上绘制剩余的源图像的像素。目标图像其他区域不变。
 */
public class XFermodeViewBitmapAtTop extends View {
    ...
    public XFermodeViewBitmapAtTop(Context context) {
        this(context, null);
    }

    public XFermodeViewBitmapAtTop(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        mDstB = makeDest(itemWidth, itemWidth);
...
//        return bm;
    ...
    Bitmap makeDest(int w, int h) {
        Bitmap bm = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);
        Canvas c = new Canvas(bm);
        Paint p = new Paint(Paint.ANTI_ALIAS_FLAG);
        Shader shader = new LinearGradient(0, 0, 0, h, new int[]{0xFF000000, 0xFF000000, 0x00000000}, null,
