...
private boolean startScrollIfNeeded(int x, int y, MotionEvent vtev) {
    ...
    if ((overscroll || distance > mTouchSlop) &&
        ...
        //（处于overscroll状态或者移动的距离足够远）&& （不是嵌套滑动状态）
...
重点方法，AbsListView的scrollIfNeeded方法。

```java
private void scrollIfNeeded(int x, int y, MotionEvent vtev) {
    //手指从下向上滑动，rawDeltaY为负数
...
AbsListView的trackMotionScroll方法。
...
1. 比如我们手指从上向下滑动，滑动了500像素，那么`incrementalDeltaY = 500`。比如ListView的getHeight()=2000，那么
`bottom = getHeight() - incrementalDeltaY = 1500`。
...
ListView正是通过这种将所以子View偏移指定的像素数来实现滚动效果的，偏移出屏幕的子View会和ListView取消关联，
并加入到RecyclerBin的mScrapViews数组中等待以后复用。

注意：这里顺便提一嘴，ScrollView是修改自身的scrollY来实现的滚动，从而显示所有的子View的，和ListView是有区别的，注意一下。


注释7处，当我们滑动的时候，会将一些字View滑出屏幕，也需要填充新的子View到屏幕中。
